...
from feemodel.tests.config import setup_tmpdatadir, poolsref
...
class PoolsOnlineTests(unittest.TestCase):
    def test_A(self):
        ...
        with setup_tmpdatadir() as datadir:
            ...
            pools_load = load_obj(
                os.path.join(datadir, 'pools', 'pe0333953.pickle'))
    ...
    def test_B(self):
        with setup_tmpdatadir() as datadir:
            print("Test B:")
            os.mkdir(os.path.join(datadir, 'pools'))
            # Test loading of saved pools
            # Ensure that the most recent is loaded
            save_obj(1, os.path.join(datadir, 'pools', 'pe0000000.pickle'))
